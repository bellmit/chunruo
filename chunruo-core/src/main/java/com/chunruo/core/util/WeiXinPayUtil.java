...
public class WeiXinPayUtil {
	...
	public static final String DES_ENCRYPT_CRYPT_KEY = "0402chunruo@163.com";
	 ...
	 * @return
	 */
	public static Map<String, String> getWeixinH5PayInfo(WeChatAppConfig weChatAppConfig, String outTradeNo, String openId, String notifyURL, int orderTotal, String body, String clientIp){
		Map<String, String> resultMap = new HashMap<String, String> ();
		// 第一步获取统一下单
		Map<String, Object> objectMap = WeiXinPayUtil.requestUnifiedOrder(weChatAppConfig, outTradeNo, openId, notifyURL, orderTotal, body, clientIp);
		if(objectMap != null 
				&& objectMap.size() > 0
				&& objectMap.containsKey("prepay_id")){
			//判断是否跨号支付
			String tradeType = StringUtil.nullToString(objectMap.get("trade_type"));
	 ...
	 * @return
	 */
	public static Map<String, String> getWeixinAppPayInfo(WeChatAppConfig weChatAppConfig, String outTradeNo, String openId, String notifyURL, int orderTotal, String body, String clientIp){
		Map<String, String> resultMap = new HashMap<String, String> ();
		// 第一步获取统一下单
		Map<String, Object> objectMap = WeiXinPayUtil.requestUnifiedOrder(weChatAppConfig, outTradeNo, openId, notifyURL, orderTotal, body, clientIp);
		if(objectMap != null 
	 ...
	 * @return
	 */
	public static MsgModel<String> getQueryPayInfo(WeChatAppConfig weChatAppConfig, String transactionId, String tradeNo){
		...
		try{
			...
			if(!StringUtil.isNull(transactionId)){
				...
				nvpList.add(new NameValuePair("out_trade_no", StringUtil.null2Str(tradeNo)));					//微信订单号
			}
	 ...
	 * @return
	 */
	public static String getWeixinBeforePayInfo(WeChatAppConfig weChatAppConfig, String outTradeNo, String openId, String notifyURL, int orderTotal, String body, String clientIp, String tradeType){
		List<NameValuePair> nvpList = new ArrayList<NameValuePair> ();
	 ...
	 * @return
	 */
	public static Map<String, Object> requestUnifiedOrder(WeChatAppConfig weChatAppConfig, String outTradeNo, String openId, String notifyURL, int orderTotal, String body, String clientIp){
		Map<String, Object> resultMap = new HashMap<String, Object> ();
		try {
			String unifiedOrderURL = "https://api.mch.weixin.qq.com/pay/unifiedorder";
			String orderXML = WeiXinPayUtil.getWeixinBeforePayInfo(weChatAppConfig, outTradeNo, openId, notifyURL, orderTotal, body, clientIp, null);
			...
			String returnCode = StringUtil.null2Str(map.get("return_code"));
			String returnMsg = StringUtil.null2Str(map.get("return_msg"));
			if(returnCode.equals("FAIL") && returnMsg.equals("appid and openid not match")){
				orderXML = WeiXinPayUtil.getWeixinBeforePayInfo(weChatAppConfig, outTradeNo, openId, notifyURL, orderTotal, body, clientIp, "NATIVE");
				resultXML = HttpClientUtil.postXML(unifiedOrderURL, orderXML);
				map = XmlParseUtil.xmlCont2Map(resultXML);
			}
			return map;
		} catch (Exception e) {
			e.printStackTrace();
...
//		            logger.info("****************退款申请成功！**********************");
	...
	public static MsgModel<Boolean> checkRefundInfo(WeChatAppConfig weChatAppConfig,String transactionId,String refundNumber,String outTradeNo){
		...
		try {
			...
			System.out.println("微信退款查询接口==========="+sb2.toString());
	 ...
	 * @return
	 */
	public static MsgModel<Boolean> transfers(WeChatAppConfig weChatAppConfig,String orderNo, Double amount,String openid,String userName){
		MsgModel<Boolean> msgModel = new MsgModel<Boolean> ();
		try {
			...
			String nonceStr = UUID.randomUUID().toString().replace("-", "");
			List<NameValuePair> nvpList = new ArrayList<NameValuePair>();
			nvpList.add(new NameValuePair("mch_appid", StringUtil.null2Str(weChatAppConfig.getAppId()))); // 公众号id
			nvpList.add(new NameValuePair("mchid", StringUtil.null2Str(weChatAppConfig.getMchId()))); // 商户号
			nvpList.add(new NameValuePair("nonce_str", StringUtil.null2Str(nonceStr)));                // 随机字符串，长度要求在32位以内。推荐随机数生成算法
			nvpList.add(new NameValuePair("partner_trade_no", StringUtil.null2Str(orderNo)));      // 商户订单号
			nvpList.add(new NameValuePair("openid", StringUtil.null2Str(openid)));                  // 用户openid
			nvpList.add(new NameValuePair("check_name", StringUtil.null2Str("FORCE_CHECK")));		       //校验用户姓名选项  NO_CHECK：不校验真实姓名 FORCE_CHECK：强校验真实姓名
			nvpList.add(new NameValuePair("re_user_name", StringUtil.null2Str(userName)));      // 收款用户姓名  收款用户真实姓名 如果check_name设置为FORCE_CHECK，则必填用户真实姓名
			nvpList.add(new NameValuePair("amount", StringUtil.null2Str(orderAmountToBranch(amount))));		       //金额
			nvpList.add(new NameValuePair("desc", "提现转账"));		       //企业付款备注
			...
			System.out.println("微信转账查询接口==========="+sb2.toString());
			...
			if(resultMap != null
				...
				if(StringUtil.compareObject("SUCCESS", StringUtil.null2Str(resultMap.get("result_code")))) {
					//转账成功
					msgModel.setIsSucc(true);
					msgModel.setData(true);
					msgModel.setMessage("转账成功");
					msgModel.setMap(resultMap);
					return msgModel;
				}else {
					//转账失败
					log.info("转账失败代码："+resultMap.get("err_code")+",错误代码描述:"+resultMap.get("err_code_des"));
					
					msgModel.setIsSucc(false);
					msgModel.setData(false);
					msgModel.setMessage(String.format("转账失败:%s", resultMap.get("err_code_des")));
					msgModel.setMap(resultMap);
					return msgModel;
				}
    ...
    */
	...
	public static Map<String, String> sign(String jsApiTicket, String url) {
        ...
        String timestamp = DateUtil.getCurrentMillisecond();
        String string1;
        String signature = "";
	...
	public static void main(String[] args) {
		...
		weChatAppConfig.setAppId("wx99c6c92ad6ecb93b");
		weChatAppConfig.setAppSecret("68d50d393385ef07565d8d49ea66ed4c");
		weChatAppConfig.setMchId("1520708841");
		weChatAppConfig.setSecretKey("oXLhcw7ulekQ0MJfBRSqGeqId380oXLh");
		weChatAppConfig.setTradeType("APP");
		
//		WeChatAppConfig weChatAppConfig = new WeChatAppConfig();
//		weChatAppConfig.setAppId("wx99c6c92ad6ecb93b");
//		weChatAppConfig.setAppSecret("68d50d393385ef07565d8d49ea66ed4c");
//		weChatAppConfig.setMchId("1405807802");
//		weChatAppConfig.setSecretKey("39c5e9e89588450ab90b9dc4f49e87ae");
//		weChatAppConfig.setTradeType("APP");
		MsgModel<Double> msgModel = getQueryRefundInfo(weChatAppConfig, "4200000334201904260935649073","2019042614521004307",514d,514d,"T2019042710331048830");
		System.out.println(msgModel.getData());
//		String totalFee = StringUtil.null2Str(resultMap.get("total_fee"));
//		totalFee = MathUtil.getMathStr(Double.parseDouble(totalFee) / 100, 2);
