...
<script type="text/javascript">
    var currentPage = 1,
        keyword = '';
    $(function() {
        sendAjax.get("widget/getProductList", {
        ...
        if (pageList && pageList.length > 0) {
            ...
            for (var i = 0, len = pageList.length; i < len; i++) {
                html += '<tr><td class="title" style="max-width: 300px;">' +
                    ...
                    '<td class="opts">' +
                    '<div class="td-cont">' +
                    '<button class="btn js-choose" onclick="selectProduct(this)" data-id="' + pageList[i].productId + '" data-title="' + pageList[i].name + '" data-price="' + pageList[i].price + '" data-image="/upload/' + pageList[i].image + '">选取</button>' +
                    '</div></td></tr>'
            }
    ...
    function selectProduct(obj) {
        ...
        if (type && type != "null" && type == 1) {
            parent.login_box_after(parent.randNum, 'good', $(obj).data('title'), $(obj).data('id'));
        } else {
