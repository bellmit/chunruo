...
Ext.define('MyExt.rechargeManager.UserRechargeManagerListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/userRecharge/list.json?status=1"/>',
				reader: {
					type : 'json',
                	root: 'data',
			...
			if(e == 'yes'){
				Ext.Ajax.request({
		        	url: '<c:url value="/userRecharge/updateStatusByManager.json"/>',
		         	method: 'post',
					scope: this,
					params:{idListGridJson: Ext.JSON.encode(rowsData), level: 2,status:2},
					...
					if(e == 'yes'){
				     	Ext.Ajax.request({
				        	url: '<c:url value="/userRecharge/updateStatusByManager.json"/>',
				         	method: 'post',
							scope: this,
							params:{idListGridJson: Ext.JSON.encode(rowsData),level: 2,status:4,refuseReason: textAreaMsg.getValue()},
				          	success: function(response){
		          				var responseObject = Ext.JSON.decode(response.responseText);
