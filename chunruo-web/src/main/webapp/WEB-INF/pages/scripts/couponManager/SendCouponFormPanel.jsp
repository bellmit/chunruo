<%@ include file="/WEB-INF/pages/scripts/taglibs.jsp"%>

Ext.define('MyExt.couponManager.SendCouponFormPanel', {
    extend : 'Ext.form.Panel',
    alias: ['widget.sendCouponForm'],
    requires : [],
 	header: false,
 	buttonAlign: 'center',
 	labelAlign: 'right',
 	labelWidth: 40,
 	bodyPadding: '5 5 0',
    defaultType: 'textfield',
    fontStyle: '<span style="font-size:14px;font-weight:bold;">{0}</span>',
    autoScroll: true,
    viewConfig: {
        stripeRows: true,
        enableTextSelection: true
    },
    
	initComponent : function(config) {
		Ext.apply(this, config);
	    
	    this.items = [{
        	xtype: 'fieldset',
            title: Ext.String.format(this.fontStyle, '<fmt:message key="order.info"/>'),
            bodyPadding: '5 5 0',
            anchor: '99%',
            layout: 'hbox',
            items:[{
                xtype: 'container',
                flex: 1,
                layout: 'anchor',
                items: [{
				    xtype:'datefield',
				    fieldLabel: '<fmt:message key="order.beginTime"/>',
				    labelWidth: 100,
				    format: 'Y-m-d H:i:s',
				    readOnly: false,
				    id:'beginTime',
				    name: 'beginTime',
				    value:Ext.Date.add(new Date(),Ext.Date.DAY,-1),
				    anchor:'97%'
				},{
				    xtype:'datefield',
				    fieldLabel: '<fmt:message key="order.endTime"/>',
				    labelWidth: 100,
				    readOnly: false,
				    format: 'Y-m-d H:i:s',
				    name: 'endTime',
				    id:'endTime',
				    value:new Date(),
				    anchor:'97%'
				},{
				    xtype:'textfield',
				    fieldLabel: '<fmt:message key="coupon.taskName"/>',
				    labelWidth: 100,
				    readOnly: false,
				    id:'taskName',
				    name: 'taskName',
				    anchor:'97%',
				},{
				    xtype:'textfield',
				    fieldLabel: '<fmt:message key="coupon.couponId"/>',
				    labelWidth: 100,
				    readOnly: false,
				    id:'couponId',
				    name: 'couponId',
				    anchor:'97%',
				}]
			   }]
        }];
        
       
    	this.callParent();
    }
});