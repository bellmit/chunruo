...
Ext.define('SystemSendMsg', {
    ...
    fields: [
    	...
    	{name: 'isInvitePage', 	type: 'bool'},
    	{name: 'objectType', 	type: 'string'},
    	{name: 'beginTime', 	type: 'string'},
    	{name: 'jumpPageType',  type: 'int'},
		{name: 'imageUrl',	    type: 'string'},
...
Ext.define('MyExt.systemSendMsg.SystemSendMsgGrid', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/pushMessage/list.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
	    ...
	    this.columns = [
            ...
            {text: '<fmt:message key="home.popup.jump.type" />', dataIndex: 'jumpPageType', width: 240, sortable: true,
            	filter: {
            		xtype: 'combobox',
			        displayField: 'name',
			        valueField: 'id',
            	...
            	renderer: this.jumpPageTypeRenderer
            },
        	...
        	{text: '<fmt:message key="web.url" />', dataIndex: 'webUrl', width: 180, sortable: true},
        ...
        this.tbar = [
        <jkd:haveAuthorize access="/pushMessage/addMessage.json">
        {
	...
	addMessage : function(){
		var addMessagePanel = Ext.create('MyExt.systemSendMsg.SystemSendMsgFormPanel', {id: 'addMessage@systemSendMsg', title: '<fmt:message key="system.sendmsg.add"/>'});
		var buttons = [{
			text: '<fmt:message key="button.save"/>',
			handler: function(){
			var levels = [];
			var items = addMessagePanel.down('checkboxgroup[name=level]').items;   
         	for (var i = 0; i < items.length; i++){    
				...
				var rowsData = [];    
		       	addMessagePanel.down('imagepanel').store.each(function(record) {
		       		record.data.input_file = null;
		            rowsData.push(record.data);    
		      	}, this);
		      	
	            if(addMessagePanel.form.isValid()){
	            	Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
			                addMessagePanel.form.submit({
			                    waitMsg: 'Loading...',
			                    url: '<c:url value="/pushMessage/addMessage.json"/>',
			                    scope: this,
		...
		openWin('<fmt:message key="system.sendmsg.add"/>', addMessagePanel, buttons, 800, 450);
	},
	...
	objectTypeRenderer: function(value, meta, record) { 
       	if(value =="1") {
            ...
            return '<fmt:message key="user.pushLevel1"/>';
        }else{
            return '<fmt:message key="user.pushLevel2"/>';
        }
   	},
    ...
    messageTypeRenderer: function(value, meta, record) { 
   	...
   	jumpPageTypeRenderer: function(value, meta, record) { 
        if(value =="0") {
            return '<b><fmt:message key="home.popup.jump.type0"/></b>';
        }else if(value == "1"){
            return '<fmt:message key="home.popup.jump.type1"/>';
        }else if(value == "3"){
            return '<fmt:message key="home.popup.jump.type3"/>';
        }  
   	},
   	    ...
   	    if(val == 0) {
            ...
            return '<b><fmt:message key="home.popup.jump.type3"/></b>';
         }
   	},
