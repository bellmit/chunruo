...
Ext.define('MyExt.activityManager.InfoPartnerListPanel', {
    ...
    initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/activity/infoList.json?level=4"/>',
				reader: {
					type : 'json',
                	root: 'data',
	    ...
	    this.columns = [
			{text: '<fmt:message key="activity.info.infoId"/>', dataIndex: 'infoId', width: 75, sortable: true,  filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="activity.info.sort"/>', dataIndex: 'sort', width: 200, sortable: true,  filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="activity.info.title"/>', dataIndex: 'title', width: 200, sortable: true,  filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="activity.info.introduce"/>', dataIndex: 'introduce', width: 200, sortable: true,  filter: {xtype: 'textfield'}},
			{
    ...
    addPartner : function(){
    var rowsData = [];		
    	var infoFormPanel = Ext.create('MyExt.activityManager.InfoPartnerForm', {
    	...
    	var buttons = [{
				...
				Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
					if(e == 'yes'){
	             		infoFormPanel.form.submit({
			                    ...
			                    scope: this,
			                    params:{level: 4,recodeGridJson: Ext.JSON.encode(rowsDatas)},
      	...
      	openWin('<fmt:message key="activity.add"/>', infoFormPanel, buttons, 500, 400);
    },
       ...
       editPartner : function(){
		...
		introduce = records[0].data.introduce;
