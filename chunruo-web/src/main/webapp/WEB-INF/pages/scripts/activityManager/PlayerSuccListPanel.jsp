...
Ext.define('MyExt.activityManager.PlayerSuccListPanel', {
	...
	initComponent : function(config) {
		...
		var now = new Date();
   		...
   		Ext.util.Cookies.set('isCheck','2',expiry);
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/activity/playerList.json?status=2"/>',
				reader: {
					type : 'json',
                	root: 'data',
        ...
        this.tbar = [{
        	...
        	text: '<fmt:message key="activity.second"/>', 
        	iconCls: 'enable', 	
        	handler: this.second, 
        	scope: this
        }];
    ...
    second : function() {
		...
		Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
			if(e == 'yes'){
				Ext.Ajax.request({
		        	url: '<c:url value="/activity/second.json"/>',
		         	method: 'post',
					scope: this,
