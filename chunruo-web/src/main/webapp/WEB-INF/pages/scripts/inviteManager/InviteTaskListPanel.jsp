...
Ext.define('MyExt.inviteManager.InviteTaskListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/inviteTask/list.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
			{text: '<fmt:message key="invite.task.taskId"/>',   dataIndex: 'taskId',   width: 60, sortable : true, filter: {xtype: 'textfield'}}, 
			{text: '<fmt:message key="invite.task.number"/>',    dataIndex: 'number',    width: 80, sortable: true,  filter: {xtype: 'textfield'}},
			 {
            	text: '<fmt:message key="invite.task.imagePath"/>',
                menuDisabled: true,
                sortable: false,
                xtype: 'actioncolumn',
                align: 'center',
             ...
             {text: '<fmt:message key="invite.task.type"/>', dataIndex: 'type',  width: 120, sortable : true,
        		align: 'center',
        		renderer: function(value, meta, record) {    
			       	if(value == 1) {
			            return '<fmt:message key="invite.task.type1"/>';
			        }else if(value == 2){
			            return '<fmt:message key="invite.task.type2"/>';
			        } else{
			            return '<fmt:message key="invite.task.type3"/>';
			        } 
			   	},
        		filter: {
					xtype: 'combobox',
        	...
        	{text: '<fmt:message key="invite.task.amount"/>',    dataIndex: 'amount',    width: 100, sortable: true,  filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="invite.task.couponIds"/>',    dataIndex: 'couponIds',    width: 120, sortable: true,  filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="invite.task.inviteDesc"/>',    dataIndex: 'inviteDesc',    width:300, sortable: true,  filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="discovery.creater.createTime"/>',  dataIndex: 'createTime',  width: 150, sortable : true, align: 'center',filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="discovery.creater.updateTime"/>',  dataIndex: 'updateTime',  width: 150, sortable : true,filter: {xtype: 'textfield'}},
        ];
        
        this.tbar = [
        <jkd:haveAuthorize access="/inviteTask/list.json">
        {
		...
		<jkd:haveAuthorize access="/inviteTask/saveInviteTask.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
	  	'-', {
        	text: '<fmt:message key="discovery.module.save"/>', 
        	iconCls: 'enable', 	
        	handler: this.saveInviteTask, 
        	scope: this
        }
        </jkd:haveAuthorize>
        <jkd:haveAuthorize access="/inviteTask/deleteInviteTask.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
	  	'-', {
        	text: '<fmt:message key="discovery.module.delete"/>', 
        	iconCls: 'delete', 	
        	handler: this.deleteInviteTask, 
        	scope: this
        }
        </jkd:haveAuthorize>
        ];
	    ...
	    this.east =  Ext.create('MyExt.inviteManager.InviteTaskTabPanel', {
	    ...
	    this.productList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
	    	<jkd:haveAuthorize access="/inviteTask/getInviteTaskById.json">
   	...
   	saveInviteTask : function(){
    	var inviteTaskFormPanel = Ext.create('MyExt.inviteManager.InviteTaskFormPanel', {
			id: 'add@inviteTaskFormPanel' + this.id,
    		viewer: this.viewer,
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
			    var rowsData = [];    
		       	inviteTaskFormPanel.down('imagepanel').store.each(function(record) {
		       		record.data.input_file = null;
		            rowsData.push(record.data);    
		      	}, this);
		    	if(inviteTaskFormPanel.form.isValid()){
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		inviteTaskFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
      	...
      	openWin('<fmt:message key="button.add"/>', inviteTaskFormPanel, buttons,400, 400);
    },    
    
    deleteInviteTask : function() {
	    ...
	    Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="enable.confirm"/>', function(e){
			if(e == 'yes'){
	        	Ext.Ajax.request({
		        	url: '<c:url value="/inviteTask/deleteInviteTask.json"/>',
		         	method: 'post',
					scope: this,
					params:{taskId : records[0].data.taskId},
