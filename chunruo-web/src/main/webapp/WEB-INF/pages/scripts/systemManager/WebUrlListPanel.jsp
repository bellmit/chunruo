...
Ext.define('MyExt.systemManager.WebUrlListPanel', {
	...
	initComponent : function(config) {
		...
		this.columns = [
			{text: '<fmt:message key="web.url.configId"/>',   dataIndex: 'configId',   width: 60, sortable : true, filter: {xtype: 'textfield'}}, 
			{text: '<fmt:message key="web.url.name"/>',    dataIndex: 'name',    width: 190, sortable: true,  filter: {xtype: 'textfield'}},
        	...
        	{text: '<fmt:message key="web.url.url"/>',    dataIndex: 'url',    width: 200, sortable: true,  filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="web.url.methodName"/>',  dataIndex: 'methodName',  width: 500, sortable : true, align: 'center',filter: {xtype: 'textfield'}},
            {text: '<fmt:message key="discovery.creater.createTime"/>',  dataIndex: 'createTime',  width: 150, sortable : true,filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="discovery.creater.updateTime"/>',  dataIndex: 'updateTime',  width: 150, sortable : true,filter: {xtype: 'textfield'}},
        ];
        
        this.tbar = [
        <jkd:haveAuthorize access="/webUrl/list.json">
        {
	    ...
	    this.east =  Ext.create('MyExt.systemManager.WebUrlTabPanel', {
	    ...
	    this.productList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
	    	<jkd:haveAuthorize access="/webUrl/getWebUrlById.json">
   	...
   	saveWebUrl : function(){
    	var webUrlFormPanel = Ext.create('MyExt.systemManager.WebUrlFormPanel', {
			id: 'add@webUrlFormPanel' + this.id,
    		viewer: this.viewer,
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
		    	...
		    	if(webUrlFormPanel.form.isValid()){
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		webUrlFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
		                 		url: '<c:url value="/webUrl/saveWebUrl.json"/>',
		               			scope: this,
		               			params:{},
      	...
      	openWin('<fmt:message key="button.add"/>', webUrlFormPanel, buttons,300, 400);
    },    
