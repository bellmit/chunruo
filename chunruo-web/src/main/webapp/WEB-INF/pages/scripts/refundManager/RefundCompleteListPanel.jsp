...
Ext.define('MyExt.refundManager.RefundCompleteListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			sortOnLoad: true,
			remoteSort: true,
			model: 'RefundComplete',
			proxy: {
				type: 'ajax',
				url: '<c:url value="/refund/list.json?status=5"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.refundCompleteListBbar = this.pagingToolbar; 
		
    	this.refundCompleteList = Ext.create('Ext.grid.GridPanel', {
			region: 'center',
			header: false,
			autoScroll: true,   
			closable: true,
		    ...
		    store: this.store,
		    bbar: this.refundCompleteListBbar,
	    ...
	    this.tbar = ['->'
	    <jkd:haveAuthorize access="/refund/exportRefundExcel.json">
	    ,{
        	text: '<fmt:message key="button.exporter.xls"/>', 
        	iconCls: 'excel', 	
        	handler: this.exportRefund, 
        	scope: this
        }
		</jkd:haveAuthorize>
		<jkd:haveAuthorize access="/refund/exportRefundExcel.json">
        ,'-',{
        	...
        	iconCls: 'excel', 	
        	handler: this.batchExportRefund, 
        	scope: this
        }
        </jkd:haveAuthorize>
        ];
        ...
        this.east =  Ext.create('MyExt.refundManager.RefundTabPanel', {
        	refundList: this.refundCompleteList,
		 	viewer: this.viewer,
		 	layout: 'card',
	        border: false,
	       	region: 'east',
    	...
    	this.items = [this.refundCompleteList, this.east];	
		this.east.hide();
    	...
    	this.store.on('beforeload', function(store, options) {
	        Ext.apply(store.proxy.extraParams, {
	        	filters: Ext.JSON.encode(this.refundCompleteList.filters)
			});
	    }, this);
	    this.store.load();
	    </jkd:haveAuthorize>
	    
	    this.gsm = this.refundCompleteList.getSelectionModel();
	    this.refundCompleteList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
    ...
    batchExportRefund : function(){
		var refundExcelPanel = Ext.create('MyExt.refundManager.RefundExportExcel', {id: 'refundExportExcel@RefundExportExcel', title: '<fmt:message key="button.add"/>'});
		var buttons = [{
			text: '<fmt:message key="button.batch.exporter.xls"/>',
			handler: function(){
	            if(refundExcelPanel.form.isValid()){
	            	Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="order.export.confirm"/>', function(e){
						if(e == 'yes'){
						var formValues=refundExcelPanel.getForm().getValues();
							...
							window.location.href = '<c:url value="/refund/exportRefundExcel.json?beginTime="/>' + begin + '&endTime=' + end;
			              	popWin.close();
			        	}
			        }, this)
	        	}
		...
		openWin('<fmt:message key="button.exporter.xls"/>', refundExcelPanel, buttons, 400, 200);
	},
	...
	exportRefund : function(){
		...
		Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="refund.receipt.confirm"/>', function(e){
			if(e == 'yes'){
				window.location.href = '<c:url value="/refund/exportRefundExcel.json?idListGridJson="/>' + Ext.JSON.encode(rowsData);
	     	}
	 	}, this) 
    },
