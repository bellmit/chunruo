...
Ext.define('MyExt.refundManager.RefundAllListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/refund/list.json?status=1,2,3,4,5,6,7,8,9"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
        	...
        	{text: '<fmt:message key="order.refund.orderId"/>', dataIndex: 'orderNo', width: 160,  sortable : true,locked: true,
        		align: 'center',filter: {xtype: 'textfield'}
        	},
	    ...
	    <jkd:haveAuthorize access="/refund/exportRefund.json">
	   	this.tbar = ['->',{
        	text: '<fmt:message key="button.exporter.refund.xls"/>', 
        	iconCls: 'excel', 	
	            ...
	            if(refundExcelPanel.form.isValid()){
	            	Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="order.export.confirm"/>', function(e){
						...
						var formValues=refundExcelPanel.getForm().getValues();
							...
							window.location.href = '<c:url value="/refund/exportRefund.json?beginTime="/>' + begin + '&endTime=' + end;
			              	popWin.close();
			        	}
			        }, this)
	        	}
