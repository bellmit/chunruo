...
Ext.define('MyExt.refundManager.RefundAgreeListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/refund/list.json?status=3"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.refundAgreeListBbar = this.pagingToolbar; 
		
    	this.refundAgreeList = Ext.create('Ext.grid.GridPanel', {
			region: 'center',
			header: false,
			autoScroll: true,   
			closable: true,
        ...
        this.east =  Ext.create('MyExt.refundManager.RefundTabPanel', {
        	refundList: this.refundAgreeList,
		 	viewer: this.viewer,
		 	layout: 'card',
	        border: false,
	       	region: 'east',
    	...
    	this.items = [this.refundAgreeList, this.east];	
		this.east.hide();
    	...
    	this.store.on('beforeload', function(store, options) {
	        Ext.apply(store.proxy.extraParams, {
	        	filters: Ext.JSON.encode(this.refundAgreeList.filters)
			});
	    }, this);
	    this.store.load();
	    </jkd:haveAuthorize>
	    
	    this.gsm = this.refundAgreeList.getSelectionModel();
	    this.refundAgreeList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
