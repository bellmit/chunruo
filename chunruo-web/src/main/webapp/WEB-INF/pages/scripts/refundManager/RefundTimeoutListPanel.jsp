...
Ext.define('MyExt.refundManager.RefundTimeoutListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/refund/list.json?status=6"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.refundTimeoutListBbar = this.pagingToolbar; 
		
    	this.refundTimeoutList = Ext.create('Ext.grid.GridPanel', {
			region: 'center',
			header: false,
			autoScroll: true,   
			closable: true,
		    ...
		    store: this.store,
		    bbar: this.refundTimeoutListBbar,
        ...
        this.east =  Ext.create('MyExt.refundManager.RefundTabPanel', {
        	refundList: this.refundTimeoutList,
		 	viewer: this.viewer,
		 	layout: 'card',
	        border: false,
	       	region: 'east',
    	...
    	this.items = [this.refundTimeoutList, this.east];	
		this.east.hide();
    	...
    	this.store.on('beforeload', function(store, options) {
	        Ext.apply(store.proxy.extraParams, {
	        	filters: Ext.JSON.encode(this.refundTimeoutList.filters)
			});
	    }, this);
	    this.store.load();
	    </jkd:haveAuthorize>
	    
	    this.gsm = this.refundTimeoutList.getSelectionModel();
	    this.refundTimeoutList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
