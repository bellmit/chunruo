...
(function($) {
    ...
    $.openPrompt.info = function(obj, call, closecall) {
        ...
        var placeholderTxt = "请输入文件标题"
        if (obj.placeholderTxt != null && obj.placeholderTxt != "" && obj.placeholderTxt != undefined)
            placeholderTxt = obj.placeholderTxt;
        var contentDiv = $('<div class="weui_dialog_bd"></div>').appendTo(dialog);
        var inputdom = $('<input type="text" class="fileTitle" id="last_product_price"  placeholder=" ' + placeholderTxt + '" value="" style="width: 100%;height: 35px;margin-bottom: 20px;margin-top: 8px;">').appendTo(contentDiv);
     ...
     * obj.content 内容
     */
    $.openNewWindow.info = function(obj) {
        var box = $('<div class="new-winodw"></div>').appendTo($("body"));
        var title = $('<div class="new-wtitle"></div>').appendTo(box);
        var content = $('<div class="win-content"></div>').appendTo(box);
        var titleinfo = $('<span>' + obj.title + '</span>').appendTo(title);
        var winclose = $('<span class="win-close">X</span>').appendTo(title);
        winclose.click(function() {
            $(".new-winodw").remove();
        });
        var contenttext = $('<p>' + obj.content + '</p>').appendTo(content);
    }
}(jQuery));