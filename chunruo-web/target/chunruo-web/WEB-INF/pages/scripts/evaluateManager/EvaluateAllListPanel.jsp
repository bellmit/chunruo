...
Ext.define('MyExt.evaluateManager.EvaluateAllListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/evaluate/list.json?status=0,1,2,3"/>',
				reader: {
					type : 'json',
                	root: 'data',
        ...
        this.tbar = [
        <jkd:haveAuthorize access="/evaluate/list.json">
        {
		...
		<jkd:haveAuthorize access="/evaluate/checkStatus.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
	  	'-',{
        	text: '<fmt:message key="order.evaluate.status3"/>', 
        	iconCls: 'enable', 	
        	handler: this.excellent, 
        	scope: this
        }
        <c:set var="isHaveAuthorize" value="true" />
		</jkd:haveAuthorize>
	    ...
	    this.productList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
	    	<jkd:haveAuthorize access="/evaluate/getEvaluateById.json">
     ...
     excellent : function() {
		...
		Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm.excellent"/>', function(e){
			if(e == 'yes'){
				Ext.Ajax.request({
		        	url: '<c:url value="/evaluate/checkStatus.json"/>',
		         	method: 'post',
					scope: this,
					params:{idListGridJson: Ext.JSON.encode(rowsData),status : 3},
