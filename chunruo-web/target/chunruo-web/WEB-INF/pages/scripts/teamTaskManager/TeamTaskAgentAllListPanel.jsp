...
Ext.define('MyExt.teamTaskManager.TeamTaskAgentAllListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/teamTask/list.json?level=3"/>',
				reader: {
					type : 'json',
                	root: 'data',
	    ...
	    this.tbar = ['->',{
        	text: '<fmt:message key="team.task.record.xls"/>', 
        	iconCls: 'excel', 	
        	handler: this.exportAgentRecord,
        	scope: this
        } ];
