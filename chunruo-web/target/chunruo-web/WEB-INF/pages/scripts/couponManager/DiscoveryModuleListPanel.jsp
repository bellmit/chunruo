...
Ext.define('MyExt.couponManager.DiscoveryModuleListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/discovery/moduleList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
        ...
        this.tbar = [
        <jkd:haveAuthorize access="/discovery/moduleList.json">
        {
		...
		<jkd:haveAuthorize access="/discovery/saveModule.json">
	    ...
	    this.east =  Ext.create('MyExt.couponManager.DiscoveryModuleTabPanel', {
	    ...
	    this.productList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
	    	<jkd:haveAuthorize access="/discovery/getModuleById.json">
   	...
   	saveModule : function(){
    	var discoveryModuleFormPanel = Ext.create('MyExt.couponManager.DiscoveryModuleFormPanel', {
			id: 'add@discoveryModuleFormPanel' + this.id,
    		viewer: this.viewer,
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
			    var rowsData = [];    
		       	discoveryModuleFormPanel.down('imagepanel').store.each(function(record) {
		       		record.data.input_file = null;
		            rowsData.push(record.data);    
		      	}, this);
		    	if(discoveryModuleFormPanel.form.isValid()){
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		discoveryModuleFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
      	...
      	openWin('<fmt:message key="button.add"/>', discoveryModuleFormPanel, buttons,300, 160);
    },    
    ...
    deleteModule : function() {
	    ...
	    Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="enable.confirm"/>', function(e){
			if(e == 'yes'){
	        	Ext.Ajax.request({
		        	url: '<c:url value="/discovery/deleteModule.json"/>',
		         	method: 'post',
					scope: this,
					params:{moduleId : records[0].data.moduleId},
