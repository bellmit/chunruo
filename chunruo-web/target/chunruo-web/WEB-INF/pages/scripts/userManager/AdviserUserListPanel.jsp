...
Ext.define('MyExt.userManager.AdviserUserListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/user/adviserList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
			{text: '<fmt:message key="user.headerImage"/>', dataIndex: 'headerImage', width: 80, sortable : false, 
		        renderer : function(val, metadata, record, rowIndex, columnIndex, store){ 
					return '<img height="20" width="20" src="' + val + '">';
				}
        	},
        	{text: '<fmt:message key="user.userId"/>', dataIndex: 'userId', width: 70,  sortable : true,
        		filter: {xtype: 'textfield'}
        	},
        	{text: '<fmt:message key="user.nickname"/>', dataIndex: 'nickname', width: 200, sortable : true, 
        		filter: {xtype: 'textfield'}
        	},
        	{text: '<fmt:message key="user.level"/>', dataIndex: 'level', width: 100, sortable : true,
        	...
        	{text: '<fmt:message key="user.mobile"/>',         dataIndex: 'mobile', width: 200, sortable : true,
        		filter: {xtype: 'textfield'}
        	},
        	{text: '<fmt:message key="user.registerTime"/>',   dataIndex: 'registerTime', width: 200, sortable: true, align: 'center',filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="user.createTime"/>',   dataIndex: 'createTime', width: 200, sortable: true, align: 'center',filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="user.updateTime"/>',   dataIndex: 'updateTime', width: 200, sortable: true, filter: {xtype: 'textfield'}}
        ];
        
        this.tbar = [
        <jkd:haveAuthorize access="/user/adviserList.json">
        {
