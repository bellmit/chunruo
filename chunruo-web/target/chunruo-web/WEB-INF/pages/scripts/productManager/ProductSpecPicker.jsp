...
Ext.define('ProductCont', {
	extend: 'Ext.data.TreeModel',
   	fields: [
    	...
    	{name: 'productSpecId', type: 'int'},
		{name: 'name',	 		type: 'string'},
		{name: 'tag',	 		type: 'string'},
    ]
});

Ext.define('MyExt.productManager.ProductSpecPicker', {
    extend: 'Ext.form.field.Picker',
    alias: 'widget.productSpecPicker',
    ...
    initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/giftProduct/wholesaleList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.pagingToolbar = new Ext.PagingToolbar({
       	...
       	this.productList = Ext.create('Ext.grid.GridPanel', {
	        ...
	        columns: [
        		...
        		{text: '<fmt:message key="product.wholesale.productSpecId"/>', dataIndex: 'productSpecId', width: 65, sortable : true,
        			filter: {xtype: 'textfield'}
        		},
        		{text: '<fmt:message key="product.wholesale.productTags"/>', dataIndex: 'productTags', width: 130, sortable : true,
        			filter: {xtype: 'textfield'}
        		}
        	]
	    });
