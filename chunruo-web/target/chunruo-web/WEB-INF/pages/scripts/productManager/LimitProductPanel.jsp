...
Ext.define('Product', {
    ...
    fields: [
		...
		{name: 'lastRepTime',		mapping: 'lastRepTime',type: 'string'},
		
    ]
});


Ext.define('MyExt.productManager.LimitProductPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/product/limitProductList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
        	...
        	{text: '<fmt:message key="product.wholesale.max.LimitNumber"/>', dataIndex: 'maxLimitNumber', width: 65, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="product.wholesale.lastRepTime"/>', dataIndex: 'lastRepTime', width: 100, sortable : true,
        		filter: {xtype: 'textfield'}
        	},
		...
		<jkd:haveAuthorize access="/product/updateRepStatus.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
        '-',{
        	text: '<fmt:message key="product.wholesale.updateRepStatus"/>', 
        	iconCls: 'enable', 	
        	handler: this.updateRepStatus, 
        	scope: this
        }
    ...
    updateRepStatus : function() {
		...
		Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
			if(e == 'yes'){
				Ext.Ajax.request({
		        	url: '<c:url value="/product/updateRepStatus.json"/>',
		         	method: 'post',
					scope: this,
