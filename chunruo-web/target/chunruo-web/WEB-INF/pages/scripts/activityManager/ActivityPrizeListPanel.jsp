...
Ext.define('Prize', {
	...
	idProperty: 'infoId',
    fields: [
		{name: 'prizeId',		mapping: 'prizeId',		type: 'int'},
...
Ext.define('MyExt.activityManager.ActivityPrizeListPanel', {
    ...
    initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/activity/prizeList.json?level=1,2,3,4"/>',
				reader: {
					type : 'json',
                	root: 'data',
	    ...
	    this.columns = [
			{text: '<fmt:message key="activity.prize.prizeId"/>', dataIndex: 'prizeId', width: 75, sortable: true, locked: true, filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="activity.prize.level"/>', dataIndex: 'level', width: 200, sortable: true, locked: true, filter: {xtype: 'textfield'}},
            {	text: '<fmt:message key="activity.prize.image"/>',
                menuDisabled: true,
                sortable: false,
                xtype: 'actioncolumn',
                align: 'center',
            ...
            {text: '<fmt:message key="activity.prize.content"/>', dataIndex: 'content', width: 200, sortable: true, locked: true, filter: {xtype: 'textfield'}},
    ...
    savePrize : function(){
    var rowsData = [];		
    	var infoFormPanel = Ext.create('MyExt.activityManager.ActivityPrizeFormPanel', {
    	...
    	var buttons = [{
				...
				Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
					if(e == 'yes'){
	             		infoFormPanel.form.submit({
			                    waitMsg: 'Loading...',
			                    url: '<c:url value="/activity/savePrize.json"/>',
			                    scope: this,
      	...
      	openWin('<fmt:message key="activity.prize.add"/>', infoFormPanel, buttons, 310, 300);
    },	
      ...
      editPrize : function(){
		...
		prizeId = records[0].data.prizeId;	
