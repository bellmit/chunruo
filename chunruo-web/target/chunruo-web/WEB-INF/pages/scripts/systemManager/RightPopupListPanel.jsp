...
Ext.define('MyExt.systemManager.RightPopupListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/homePopup/list.json?type=1"/>',
				reader: {
					type : 'json',
                	root: 'data',
	    ...
	    this.east =  Ext.create('MyExt.systemManager.RightPopupTabPanel', {
    ...
    saveHomePopup : function(){
    	var rightPopupFormPanel = Ext.create('MyExt.systemManager.RightPopupFormPanel', {
			id: 'rightPopupFormPanel@' + this.id,
    		viewer: this.viewer,
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
		    	if(rightPopupFormPanel.form.isValid()){
			    	var rowsDatas = [];
			       	rightPopupFormPanel.down('imagepanel').store.each(function(record) {
			       		record.data.input_file = null;
			            rowsDatas.push(record.data);    
			      	}, this);
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		rightPopupFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
      	...
      	openWin('<fmt:message key="home.popup.add"/>', rightPopupFormPanel, buttons, 420, 400);
    },
