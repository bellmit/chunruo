...
Ext.define('MyExt.orderManager.OrderInterceptPanel', {
	...
	initComponent : function(config) {
        ...
        this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/order/orderInterceptList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
        	...
        	{text: '<fmt:message key="order.buyWayType"/>', dataIndex: 'buyWayType', width: 80, sortable : true,
        		align: 'center',
        		renderer: this.rendererBuyWayType,
        		filter: {
					xtype: 'combobox',
			        displayField: 'name',
			        valueField: 'id',
			        store: this.buyWayTypeStore,
			        queryMode: 'local',
			        typeAhead: true
				}
        	},
	    ...
	    this.tbar = [
	    <jkd:haveAuthorize access="/order/orderClose.json">
	    {
        	text: '<fmt:message key="order.button.close"/>', 
        	iconCls: 'delete', 	
		...
		<jkd:haveAuthorize access="/order/orderRestore.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
        '-',{
        	text: '<fmt:message key="order.button.restore"/>', 
        	iconCls: 'app_manager', 	
		...
		<jkd:haveAuthorize access="/order/orderInterceptList.json?isExporter=true">
		<c:if test="${isHaveAuthorize}">,</c:if>
        '->',{
    	...
    	<jkd:haveAuthorize access="/order/orderInterceptList.json">
    ...
    orderRestore : function(){
		...
		var textAreaMsg = Ext.create('Ext.form.TextArea', {
 			fieldLabel: '<fmt:message key="order.restore.confirm"/>',
 			labelAlign: 'top',
    		allowBlank: false,
       		anchor: '100%'
 		});	
	 	...
	 	var buttons = [{ 	
	        ...
	        handler: function(){
				if(textAreaMsg.getValue() == null || textAreaMsg.getValue().length == 0){ 
					showWarnMsg('<fmt:message key="order.restore.confirm"/>', 8);
					return;
				}
				
		     	Ext.Ajax.request({
		        	url: '<c:url value="/order/orderRestore.json"/>',
		         	method: 'post',
					scope: this,
