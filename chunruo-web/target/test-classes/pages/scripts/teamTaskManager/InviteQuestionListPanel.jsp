...
Ext.define('MyExt.teamTaskManager.InviteQuestionListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/teamTask/ruleQuestionlist.json?type=4"/>',
				reader: {
					type : 'json',
                	root: 'data',
					...
					Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
					     	teamTaskRuleForm.form.submit({
		                 		...
		                 		url: '<c:url value="/teamTask/saveOrUpdateRule.json"/>',
		               			scope: this,
		               			params:{type: 4,explainId:explainId,level:2},
		               			success: function(form, action) {
		                   			var responseObject = Ext.JSON.decode(action.response.responseText);
		                   			if(responseObject.error == false){
		                  				showSuccMsg(responseObject.message);
