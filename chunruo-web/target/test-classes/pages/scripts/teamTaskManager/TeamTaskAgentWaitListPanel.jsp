...
Ext.define('MyExt.teamTaskManager.TeamTaskAgentWaitListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/teamTask/list.json?level=3&taskStatus=4&isSignContract=1"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
        	...
        	{text: '<fmt:message key="team.task.record.isSignContract"/>', dataIndex: 'isSignContract', width: 140, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="team.task.record.examineTime2"/>', dataIndex: 'examineTime', width: 140, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
	    ...
	    this.tbar = [{
        	text: '<fmt:message key="team.task.record.sign"/>', 
            iconCls: 'enable', 
            scope: this,
        	handler: this.sign, 
        	
        },{
        	text: '<fmt:message key="team.task.record.nosign"/>', 
            iconCls: 'delete',
        	handler: this.nosign, 
        	scope: this
        }];
    ...
    sign : function(){
      var rowsData = [];
       ...
       Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="team.task.record.sign.comfirm"/>', function(e){
			if(e == 'yes'){
				Ext.Ajax.request({
		        	url: '<c:url value="/teamTask/signContract.json"/>',
		         	method: 'post',
					scope: this,
					params:{isSign:true,recordId:recordId},
		          	success: function(response){
          				var responseObject = Ext.JSON.decode(response.responseText);
                        if(responseObject.success == true){
                        	showSuccMsg(responseObject.message);
     ...
     var rowsData = [];
	   ...
	   var recordId=records[0].data.recordId;
       Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="team.task.record.nosign.comfirm"/>', function(e){
			if(e == 'yes'){
				Ext.Ajax.request({
		        	url: '<c:url value="/teamTask/signContract.json"/>',
		         	method: 'post',
					scope: this,
					params:{isSign:false,recordId:recordId},
		          	success: function(response){
          				var responseObject = Ext.JSON.decode(response.responseText);
                        if(responseObject.success == true){
                        	showSuccMsg(responseObject.message);
						...
						if(e == 'yes'){
					     	teamTaskRuleForm.form.submit({
		                 		...
		                 		url: '<c:url value="/teamTask/saveOrUpdateRule.json"/>',
		               			scope: this,
		               			params:{type: 1,explainId:explainId},
