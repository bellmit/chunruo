...
Ext.define('Voter', {
    ...
    fields: [
		...
		{name: 'voterName',	        mapping: 'voterName',          type: 'string'},
		...
		{name: 'luckName',	 	    mapping: 'luckName',           type: 'string'},
		{name: 'createTime',	 	mapping: 'createTime',		   type: 'string'},
		{name: 'updateTime',	 	mapping: 'updateTime',		   type: 'string'},
    ]
});

Ext.define('MyExt.activityManager.WinningVoterListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/activity/luckRecordList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
		    {text: '<fmt:message key="acitivity.voter.recordId"/>', dataIndex: 'recordId', width: 65, sortable: true, locked: true,align: 'center', filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="acitivity.voter.votersId"/>', dataIndex: 'voterId', width: 100, sortable: true, locked: true, align: 'center',filter: {xtype: 'textfield'}},
			...
			{text: '<fmt:message key="acitivity.voter.luckId"/>', dataIndex: 'luckId', width: 65, sortable: true, locked: true, align: 'center',filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="acitivity.voter.voterName"/>', dataIndex: 'voterName', width: 300, sortable: true, locked: true,align: 'center' },
        	{text: '<fmt:message key="acitivity.voter.luckName"/>', dataIndex: 'luckName', width: 300, sortable: true, locked: true, align: 'center'},
        	{text: '<fmt:message key="acitivity.voter.mobile"/>', dataIndex: 'mobile', width: 200, sortable: true, locked: true, align: 'center'}, 
        	{text: '<fmt:message key="acitivity.address"/>', dataIndex: 'address', width: 350, sortable : true,locked: true,align: 'center'}, 
			{text: '<fmt:message key="acitivity.createTime"/>', dataIndex: 'createTime', width: 200, sortable : true, locked: true,align: 'center',filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="acitivity.updateTime"/>', dataIndex: 'updateTime', width: 200, sortable : true,locked: true,align: 'center',filter: {xtype: 'textfield'}},
        ];
	  	...
	  	this.tbar = [{
            ...
            text: '<fmt:message key="activity.luck.record.export"/>', 
        	iconCls: 'excel', 	
    ...
    sendStatus : function(){
   	...
   	exportRecord:function(){	
           	Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="order.export.confirm"/>', function(e){
				if(e == 'yes'){
					window.location.href = "/activity/exportLuckRecordExcel.json";
	              	popWin.close();
	        	}
	        }, this)
