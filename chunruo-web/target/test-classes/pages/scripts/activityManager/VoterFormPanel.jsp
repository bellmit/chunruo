...
Ext.define('MyExt.activityManager.VoterFormPanel', {
    extend : 'Ext.form.Panel',
    alias: ['widget.voterForm'],
	...
	initComponent : function(config) {
	    ...
	    this.items = [{
			xtype: 'fieldset',
			title: '<fmt:message key="acitivity.voter.detail"/>',
           	anchor: '98%',
           	items: [{
				xtype: 'textfield',
		    	fieldLabel: '<fmt:message key="acitivity.voter.voterId" />',
		    	labelWidth: 70,
				name: 'voterId',
	         	readOnly: true,
	         	anchor: '95%'
				...
				xtype: 'textfield',
		    	fieldLabel: '<fmt:message key="acitivity.voter.userId" />',
		    	labelWidth: 70,
				name: 'userId',
	         	readOnly: true,
	         	anchor: '95%'
			    ...
			    {
				    xtype:'textfield',
				    fieldLabel: '<fmt:message key="acitivity.voter.name"/>',
				    labelWidth: 85,
				    name: 'name',
				    anchor:'97%'
				},
			    {
				    xtype:'textfield',
				    fieldLabel: '<fmt:message key="acitivity.voter.mobile"/>',
				    labelWidth: 85,
				    name: 'mobile',
				    anchor:'97%'
				},{
				xtype: 'textfield',
		    	fieldLabel: '<fmt:message key="acitivity.voter.address" />',
		    	labelWidth: 70,
				name: 'address',
	         	anchor: '95%'
				},{
		...
		var isCheck = Ext.util.Cookies.get('isCheck');
	        ...
	        handler: function(){
	        	...
	        	var voterId = formValues["voterId"];
	        	...
	        	if(this.form.isValid()){
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
			                this.form.submit({
			                    waitMsg: 'Loading...',
			                    url: '<c:url value="/activity/editVoter.json"/>',
			                    scope: this,
			                    params:{voterId : voterId, name : name, mobile : mobile, address : address},
			                    success: function(form, action) {
			                        var responseObject = Ext.JSON.decode(action.response.responseText);
			                        if(responseObject.success == true){
			                       		showSuccMsg(responseObject.message);
                                     //   this.tabPanel.loadData();
                                    Ext.StoreMgr.get('voterStore').reload();
		                       		    popWin.close();
