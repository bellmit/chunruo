...
Ext.define('PurchaseDoubt', {
	extend: 'Ext.data.Model',
	idProperty: 'doubtId',
     fields: [
    	{name: 'doubtId',	    mapping: 'doubtId',	    type: 'int'},
...
Ext.define('MyExt.productManager.PurchaseDoubtListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/purchaseDoubt/list.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
			{text: '<fmt:message key="product.intro.introId"/>', dataIndex: 'doubtId', width: 80, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="product.intro.sort"/>', dataIndex: 'sort', width: 80, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="product.intro.title"/>', dataIndex: 'title', width: 200, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="product.intro.description"/>', dataIndex: 'description', flex: 1, sortable : true,
        	filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
	    ...
	    this.tbar = [
	    <jkd:haveAuthorize access="/purchaseDoubt/saveOrUpdateDoubt.json">
	    {
        	text: '<fmt:message key="purchase.doubt.add"/>', 
            iconCls: 'add', 
            handler: this.addPurchaseDoubt,
            scope: this
        }
        ...
        {
        	text: '<fmt:message key="purchase.doubt.edit"/>', 
            iconCls: 'Chartpieadd',
        	handler: this.edit, 
        	scope: this
        }
		...
		<jkd:haveAuthorize access="/purchaseDoubt/deleteDoubt.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
        {
        	text: '<fmt:message key="purchase.doubt.delete"/>', 
            iconCls: 'delete',
        	handler: this.delete, 
        	scope: this
        }
    	...
    	<jkd:haveAuthorize access="/purchaseDoubt/list.json">
    ...
    addPurchaseDoubt : function(){
        var rowsData = [];
		var records = this.gsm.getSelection();
		var purchaseDoubtFormPanel = Ext.create('MyExt.productManager.PurchaseDoubtFormPanel', {
			id: 'purchaseDoubtFormPanel@' + this.id,
    		viewer: this.viewer,	
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
				if(purchaseDoubtFormPanel.form.isValid()){
					Ext.Msg.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
					     	purchaseDoubtFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
		                 		url: '<c:url value="/purchaseDoubt/saveOrUpdateDoubt.json"/>',
		               			scope: this,
      	...
      	openWin(Ext.String.format('<fmt:message key="product.intro.add"/>'), purchaseDoubtFormPanel, buttons, 400, 290);
    },
    ...
    edit : function(){
		...
		var doubtId=records[0].data.doubtId;
    	...
    	var buttons = [{
      	...
      	openWin(Ext.String.format('<fmt:message key="product.wareHouse.edit"/>'), purchaseDoubtFormPanel, buttons, 400, 320);
    },
