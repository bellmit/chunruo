...
Ext.define('MyExt.couponManager.DiscoveryCreaterListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			sortOnLoad: true,
			remoteSort: true,
			model: 'DiscoveryCreater',
			proxy: {
				type: 'ajax',
				url: '<c:url value="/discovery/createrList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
			{text: '<fmt:message key="discovery.creater.createrId"/>',   dataIndex: 'createrId',   width: 60, sortable : true, filter: {xtype: 'textfield'}}, 
			{text: '<fmt:message key="discovery.creater.name"/>',    dataIndex: 'name',    width: 190, sortable: true,  filter: {xtype: 'textfield'}},
			{
            ...
            {
            	text: '<fmt:message key="discovery.creater.backgroundImage"/>',
                menuDisabled: true,
                sortable: false,
                xtype: 'actioncolumn',
                align: 'center',
             ...
             {text: '<fmt:message key="discovery.creater.isRecommend"/>', dataIndex: 'isRecommend',  width: 120, sortable : true,
        		align: 'center',
            ...
            {text: '<fmt:message key="discovery.creater.isEnable"/>', dataIndex: 'isEnable',  width: 120, sortable : true,
        		align: 'center',
        		renderer: function(value, meta, record) {    
			       	if(value == false) {
			            return '<span style="color:green;"><fmt:message key="button.no"/></span>';
        	...
        	{text: '<fmt:message key="discovery.creater.introduce"/>',    dataIndex: 'introduce',    width: 500, sortable: true,  filter: {xtype: 'textfield'}},
        ...
        this.tbar = [
        <jkd:haveAuthorize access="/discovery/createrList.json">
        {
		...
		<jkd:haveAuthorize access="/discovery/saveCreater.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
	  	'-', {
        	text: '<fmt:message key="discovery.creater.save"/>', 
        	iconCls: 'enable', 	
	    ...
	    this.east =  Ext.create('MyExt.couponManager.DiscoveryCreaterTabPanel', {
	    ...
	    this.productList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
	    	<jkd:haveAuthorize access="/discovery/getCreaterById.json">
   	...
   	saveCreater : function(){
    	var discoveryCreaterFormPanel = Ext.create('MyExt.couponManager.DiscoveryCreaterFormPanel', {
			id: 'add@discoveryCreaterFormPanel' + this.id,
    		viewer: this.viewer,
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
		    	var rowsDatas = [];    
		    	var tagNameDatas = [];
		       	discoveryCreaterFormPanel.down('imagepanel').store.each(function(record) {
		       		record.data.input_file = null;
		            rowsDatas.push(record.data);    
		      	}, this);
		      	
		      	var rowsData = [];    
		       	discoveryCreaterFormPanel.down('imagepanels').store.each(function(record) {
		       		record.data.input_file = null;
		            rowsData.push(record.data);    
		      	}, this);
		      
		    	if(discoveryCreaterFormPanel.form.isValid()){
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		discoveryCreaterFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
		                 		url: '<c:url value="/discovery/saveCreater.json"/>',
		               			scope: this,
		               			params:{recodeGridJson: Ext.JSON.encode(rowsDatas),bgRecodeGridJson:Ext.JSON.encode(rowsData)},
      	...
      	openWin('<fmt:message key="button.add"/>', discoveryCreaterFormPanel, buttons,300, 400);
    },    
    
    deleteCreater : function() {
	    ...
	    Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="enable.confirm"/>', function(e){
			if(e == 'yes'){
	        	Ext.Ajax.request({
		        	url: '<c:url value="/discovery/deleteCreater.json"/>',
		         	method: 'post',
					scope: this,
					params:{createrId : records[0].data.createrId},
