...
Ext.define('MyExt.couponManager.UserLimitListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/purchaseLimit/list.json?type=2"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
        	...
        	{text: '<fmt:message key="purchase.limit.limitNumber"/>', dataIndex: 'limitNumber', width: 80,  sortable : true,locked: true,
        		align: 'center',filter: {xtype: 'textfield'}
        	},
        	{text: '<fmt:message key="purchase.limit.hours"/>', dataIndex: 'hours', width: 80, locked: true, sortable : true,
        		align: 'center'
        	},
    ...
    savePurchaseLimit : function(){
       ...
       if(records.length > 1 ){
		...
		if(records.length == 1){
			...
			hours  =  records[0].data.hours;
		}
		
    	var userLimitFormPanel = Ext.create('MyExt.couponManager.UserLimitFormPanel', {
			id: 'add@userLimitFormPanel' + this.id,
    		viewer: this.viewer,
    	...
    	var buttons = [{
			...
			handler: function(){
		    	if(userLimitFormPanel.form.isValid()){
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		userLimitFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
      	...
      	openWin('<fmt:message key="button.add"/>', userLimitFormPanel, buttons,200, 200);
    },    
