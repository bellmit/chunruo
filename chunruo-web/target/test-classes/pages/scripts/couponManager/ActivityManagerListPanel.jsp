...
Ext.define('Activity', {
	...
	idProperty: 'introId',
     fields: [
		...
		{name: 'isEnable',	        mapping: 'isEnable',        type: 'string'},
...
Ext.define('MyExt.couponManager.ActivityManagerListPanel', {
	...
	initComponent : function(config) {
		...
		this.statusStore = Ext.create('Ext.data.Store', {
        	...
        	model: 'InitModel',
        	data: [
        	    {id: 4, name: '<fmt:message key="activity.statusName4"/>'},
        		{id: 3, name: '<fmt:message key="activity.statusName3"/>'},
        		{id: 2, name: '<fmt:message key="activity.statusName2"/>'},
        		{id: 1, name: '<fmt:message key="activity.statusName1"/>'},
        	]
        });
		...
		this.columns = [
			{text: '<fmt:message key="activity.activityId"/>', dataIndex: 'activityId', width: 140, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="activity.isEnable"/>', dataIndex: 'isEnable', width: 300, sortable : true,
        	...
        	{text: '<fmt:message key="activity.startTime"/>', dataIndex: 'startTime', width: 300, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="activity.endTime"/>', dataIndex: 'endTime', width: 300, sortable : true,
        	filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="activity.statusName"/>', dataIndex: 'status', width: 80,   align: 'center', sortable : true,
        	...
        	{text: '<fmt:message key="activity.createTime"/>', dataIndex: 'createTime', width: 300, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	},
        	{text: '<fmt:message key="activity.updateTime"/>', dataIndex: 'updateTime', width: 300, sortable : true,
        		filter: {xtype: 'textfield'},
        		renderer: this.fontRenderer
        	}
        ];
	    ...
	    this.east =  Ext.create('MyExt.couponManager.ActivityManagerTabPanel', {
    ...
    sendStatus : function(){
   	...
   	statusRenderer : function(val){
   	    if(val == 1) {
            return '<b><fmt:message key="activity.statusName1"/></b>';
         }else if(val == 2){
            return '<b><fmt:message key="activity.statusName2"/></b>';
         }else if(val == 3){
            return '<b><fmt:message key="activity.statusName3"/></b>';
         }else if(val == 4){
            return '<b><fmt:message key="activity.statusName4"/></b>';
         }
   	},
