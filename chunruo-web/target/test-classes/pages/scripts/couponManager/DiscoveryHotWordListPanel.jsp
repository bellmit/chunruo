...
Ext.define('MyExt.couponManager.DiscoveryHotWordListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/discovery/hotWordList.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
			{text: '<fmt:message key="discovery.hot.wordId"/>', dataIndex: 'wordId', width: 70, locked: true, sortable : true,
        		align: 'center'
        	},
        	{text: '<fmt:message key="discovery.hot.hotWord"/>', dataIndex: 'hotWord', flex: 1,  sortable : true,locked: true,
	    ...
	    this.tbar = [
	    <jkd:haveAuthorize access="/discovery/saveHotWord.json">
	    {
        	text: '<fmt:message key="button.add"/>', 
        	iconCls: 'add', 
        	handler: this.saveHotWord, 
        	scope: this
        }
        <c:set var="isHaveAuthorize" value="true" />
		</jkd:haveAuthorize>
		<jkd:haveAuthorize access="/discovery/deleteHotWord.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
        '-',{
        	text: '<fmt:message key="button.delete"/>', 
        	iconCls: 'delete',	
        	handler: this.deleteHotWord, 
        	scope: this
        }
        </jkd:haveAuthorize>
        ];
    	...
    	<jkd:haveAuthorize access="/discovery/hotWordList.json">
