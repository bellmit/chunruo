...
Ext.define('MyExt.userManager.ApplyAgentUnApprovePanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/applyAgent/list.json?status=0"/>',
				reader: {
					type : 'json',
                	root: 'data',
    ...
    approve : function(){
		...
		var applyId=records[0].data.applyId;
		var userId=records[0].data.userId;
    	var userVipRecordFormPanel = Ext.create('MyExt.userManager.UserVipRecordFormPanel', {
			id: 'userVipRecordFormPanel@' + this.id,
    		viewer: this.viewer,	
   	 	});
    	...
    	var buttons = [{
			...
			handler: function(){
				...
				var tradeNo='';
				var paymentType='';
		    	userVipRecordFormPanel.items.each(function(form) {
	        		if(!form.isValid()){
	        			isCheckSucc = false;
	        		}else{
	        			 recordType=  Ext.getCmp('recordType').value;	 
	        			 cost= Ext.getCmp('cost').getValue();
	        			 tradeNo= Ext.getCmp('tradeNo').getValue();
	        			 paymentType= Ext.getCmp('paymentType').value;
	        		}
				}, this);
				if(!isCheckSucc){
					showMsg('<fmt:message key="ajax.confirm"/>', '<fmt:message key="ajax.no.record"/>');
					...
					if(e == 'yes'){
				     	Ext.Ajax.request({
				        	url: '<c:url value="/applyAgent/approve.json"/>',
				         	method: 'post',
							scope: this,
							params:{applyId:applyId,userId: userId,recordType: recordType,cost:cost,tradeNo:tradeNo,paymentType:paymentType,tag:1},
				          	success: function(response){
		          				var responseObject = Ext.JSON.decode(response.responseText);
		          				if(responseObject.success == true){
		                       		showSuccMsg(responseObject.message);
      	...
      	openWin(Ext.String.format('<fmt:message key="user.vipRecord"/>'), userVipRecordFormPanel, buttons, 300, 250);
    },
