...
Ext.define('UserInfo', {
    ...
    fields: [
		...
		{name: 'curDaySaleAmount',	 	mapping: 'curDaySaleAmount',	type: 'string'},
		{name: 'curMonthSaleAmount',	mapping: 'curMonthSaleAmount',	type: 'string'},
		{name: 'lastMonthSaleAmount',	mapping: 'lastMonthSaleAmount', type: 'string'},
		{name: 'lastMonthRefundAmount',	mapping: 'lastMonthRefundAmount',type: 'string'}
    ]
});

Ext.define('MyExt.userManager.UserSaleRecordListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/userSale/list.json"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.columns = [
			{text: '<fmt:message key="user.headerImage"/>', dataIndex: 'headerImage', width: 50, sortable : false, locked: true,
        	...
        	{text: '<fmt:message key="user.curDaySaleAmount"/>', dataIndex: 'curDaySaleAmount', width: 120, sortable : true,
        		filter: {xtype: 'textfield'}
        	},
        	{text: '<fmt:message key="user.curMonthSaleAmount"/>',   dataIndex: 'curMonthSaleAmount', width: 150, sortable: true, align: 'center',filter: {xtype: 'textfield'}},
        	{text: '<fmt:message key="user.lastMonthSaleAmount"/>',   dataIndex: 'lastMonthSaleAmount', width: 150, sortable: true, filter: {xtype: 'textfield'}},
			{text: '<fmt:message key="user.lastMonthRefundAmount"/>',           dataIndex: 'lastMonthRefundAmount', width: 500, sortable : true,
        		filter: {xtype: 'textfield'}
        	}
        ];
        
        this.tbar = [
        <jkd:haveAuthorize access="/userSale/list.json">
        {
		...
		<jkd:haveAuthorize access="/userSale/exportUserSaleExcel.json">
		<c:if test="${isHaveAuthorize}">,</c:if>
	  	'->',{
        	text: '<fmt:message key="user.sale.xls"/>',
        	iconCls: 'excel', 	
        	handler: this.exportUserRecord,
        	scope: this
        }
        </jkd:haveAuthorize>
        ];
    ...
    exportPushUser : function(){
      ...
      exportUserRecord : function(){
		var orderExcelPanel = Ext.create('MyExt.userManager.UserSaleExportExcel', {id: 'UserSaleExportExcel@UserSaleExportExcel', title: '<fmt:message key="button.add"/>'});
		var buttons = [{
			text: '<fmt:message key="user.sale.xls"/>',
			handler: function(){
	            if(orderExcelPanel.form.isValid()){
	            	Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="order.export.confirm"/>', function(e){
						if(e == 'yes'){
							...
							window.location.href = "/userSale/exportUserSaleExcel.json?beginTime="+begin+"&endTime="+end+"&phone=" + phone;
			              	popWin.close();
			        	}
			        }, this)
	        	}
		...
		openWin('<fmt:message key="user.sale.xls"/>', orderExcelPanel, buttons, 400, 200);
	}, 
