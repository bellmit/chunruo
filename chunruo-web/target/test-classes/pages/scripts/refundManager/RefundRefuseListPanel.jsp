...
Ext.define('MyExt.refundManager.RefundRefuseListPanel', {
	...
	initComponent : function(config) {
		...
		this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/refund/list.json?status=2"/>',
				reader: {
					type : 'json',
                	root: 'data',
		...
		this.refundRefuseListBbar = this.pagingToolbar; 
		
    	this.refundRefuseList = Ext.create('Ext.grid.GridPanel', {
			region: 'center',
			header: false,
			autoScroll: true,   
			closable: true,
		    ...
		    store: this.store,
		    bbar: this.refundRefuseListBbar,
        ...
        this.east =  Ext.create('MyExt.refundManager.RefundTabPanel', {
        	refundList: this.refundRefuseList,
		 	viewer: this.viewer,
		 	layout: 'card',
	        border: false,
	       	region: 'east',
    	...
    	this.items = [this.refundRefuseList, this.east];	
		this.east.hide();
    	...
    	this.store.on('beforeload', function(store, options) {
	        Ext.apply(store.proxy.extraParams, {
	        	filters: Ext.JSON.encode(this.refundRefuseList.filters)
			});
	    }, this);
	    this.store.load();
	    </jkd:haveAuthorize>
	    
	    this.gsm = this.refundRefuseList.getSelectionModel();
	    this.refundRefuseList.on('itemdblclick', function(view, record, item, index, e, eOpts) {
