...
Ext.define('MyExt.imManager.CommonUserFriendListPanel', {
	...
	initComponent : function(config) {
    	...
    	this.store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<c:url value="/userFriend/list.json?type=1"/>',
				reader: {
					type : 'json',
                	root: 'data',
        ...
        this.tbar = [
        <jkd:haveAuthorize access="/userFriend/list.json?type=1">
        {
    ...
    updateOperator : function(){
    	var bdOperatorFormPanel = Ext.create('MyExt.systemManager.BdOperatorFormPanel', {
					...
					Ext.MessageBox.confirm('<fmt:message key="ajax.confirm"/>', '<fmt:message key="save.confirm"/>', function(e){
						if(e == 'yes'){
		             		bdOperatorFormPanel.form.submit({
		                 		waitMsg: 'Loading...',
		                 		url: '<c:url value="/bdOperator/saveBdOperator.json"/>',
		               			scope: this,
		               			params:{},
